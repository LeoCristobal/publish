...
if (isset($_GET['fetch'])) {
    ...
    $sql = 'SELECT access_log.timestamp, access_log.rfid_id, access_log.action, user_info.name 
            FROM access_log 
            LEFT JOIN user_info ON access_log.user_id = user_info.user_id 
...
.main-card { max-width: 1200px; margin: 40px auto; box-shadow: 0 2px 16px rgba(0,0,0,0.07); border-radius: 16px; }
</style>
<script>
...
This page shows the history of RFID taps, including the date/time, UID, user name, and action.
</div>
